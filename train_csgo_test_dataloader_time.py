...
def train(attn_implementation=None):
    ...
    def profile_dataloader(trainer, num_batches=50):
        logging.info("🕵️ Starting DataLoader Profiling...")
        ...
        for i, batch in tqdm(enumerate(dataloader), total=len(dataloader)):
            # 这里什么都不做，纯粹测数据拉取速度
            if i == 10:
                start_time = time.time() # 忽略第一个 batch 的初始化时间
            if i == num_batches + 10:
                break

        end_time = time.time()
        avg_time = (end_time - start_time) / num_batches
        logging.info(f"⏱️ DataLoader Avg Time per Batch: {avg_time:.4f} seconds")

        # 算笔账：如果 avg_time 大于你模型跑一个 step 的时间，那 GPU 必定会掉到 0
        return avg_time

    profile_dataloader(trainer) # 取消注释来运行测试
