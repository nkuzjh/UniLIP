        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        for decoder_layer in self.layers[: self.config.num_hidden_layers]:
            ...
            if self.gradient_checkpointing and self.training:
                # 需要定义一个闭包来适配 checkpoint 函数的要求
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        # inputs 的顺序必须和 layer.forward 的参数顺序一致
                        return module(*inputs, adarms_cond=adarms_cond, output_attentions=False)
                    return custom_forward

                # 注意：checkpoint 只能传递 Tensor 参数。
                # position_embeddings, attention_mask 等如果是 Tensor 没问题。
                # 如果有 None 或非 Tensor，可能会报错，需要 carefully handle。
                # 这里假设主要输入都是 Tensor。
                hidden_states = torch.utils.checkpoint.checkpoint(
                    ...
                    position_ids,
                    past_key_values,
                    # None, # output_attentions (占位)
                    use_cache,
                    cache_position,
                    position_embeddings,
                    use_reentrant=False # [关键] 显式关闭 reentrant
                    # adarms_cond 已经通过闭包传入了，或者也可以作为 checkpoint 的参数传入
                    # use_reentrant=False # 建议显式设置，避免新版 PyTorch 警告
                )
            else:
